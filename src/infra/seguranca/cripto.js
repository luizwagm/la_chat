/* ==========================================================================
   ...
   O banco guarda nome, telefone, e-mail e CPF de quem procura imóvel, mais o
   ...
   pasta, ou acesso de leitura ao Postgres, leva a carteira de clientes inteira
   — que numa imobiliária é o ativo, e vale para o concorrente.

   Agora esses campos são cifrados PELA APLICAÇÃO, antes de chegarem ao banco.
   A chave NÃO mora no banco — mora no ambiente do serviço (/etc/imobiliaria.env).
   ...
   vez de devolver lixo silenciosamente — num valor de proposta, lixo silencioso é
   pior que erro.

   Cada valor usa um IV (vetor de inicialização) ALEATÓRIO próprio. É isso que
   faz dois leads com o mesmo telefone terem textos cifrados diferentes —
...
function carregarChaves() {
  ...
  /* ------------------------------------------------------------------------
     NOME PRÓPRIO, COM DESVIO PARA O DO HOSPEDEIRO.

     Este módulo vive DENTRO de sites que já usam `DADOS_CHAVE` para os dados
     deles. Se o chat lesse a mesma variável, duas coisas ruins aconteceriam:
     a chave do cliente passaria a cifrar mensagens (misturando dois domínios
     de segurança que devem poder ser rotacionados em separado), e rotacionar
     a do site decifraria errado o chat inteiro.

     Então a variável do chat é `CHAT_DADOS_CHAVE`. O desvio para `DADOS_CHAVE`
     existe só para a instalação de quem roda o chat no MESMO processo do site
     e não quer um segundo segredo — é conveniência consciente, não descuido.
     ------------------------------------------------------------------------ */
  const atual = process.env.CHAT_DADOS_CHAVE || process.env.DADOS_CHAVE;
  const anterior = process.env.CHAT_DADOS_CHAVE_ANTERIOR || process.env.DADOS_CHAVE_ANTERIOR;
  try {
    if (!atual) throw new Error("CHAT_DADOS_CHAVE não está definida no ambiente");
    mapa[VERSAO_ATUAL] = ler(atual, "CHAT_DADOS_CHAVE");
    if (anterior) mapa["0"] = ler(anterior, "CHAT_DADOS_CHAVE_ANTERIOR");
    CHAVES = mapa;
    ERRO_CHAVE = "";
  } catch (e) {
...
/* ==========================================================================
   ...
   gera textos diferentes a cada gravação. Para o caso "achar a pessoa por
